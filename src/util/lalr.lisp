...
(defun build-parser (name)
  ...
  `(DEFUN ,name (NEXT-INPUT PARSE-ERROR)
     (LET ((CAT-LA '())						;category lookahead
	   ...
	   (CUR-STATE ',(state-name (first stateList))))	;current state
       (LABELS ((INPUT-PEEK ()
		  (UNLESS CAT-LA
                    (MULTIPLE-VALUE-SETQ (CAT-LA VAL-LA) (FUNCALL NEXT-INPUT))
		    (SETF CAT-LA (LIST CAT-LA)
			  VAL-LA (LIST VAL-LA)))
		  (FIRST CAT-LA))
                (SHIFT-FROM (NAME)
...
(defun translate-state (state)
  ...
  (let ((reduces (compact-items 
       ...
       (labels ((translate-shift (shift)
		  ...
		  `(,(shift-cat shift)	;case key
		    (SHIFT-FROM ',(state-name state))
		    (SETQ CUR-STATE ',(shift-where shift))))
		
                (translate-reduce (item)
		  (when (intersection (item-la item) symbols-sofar)
		    (format t "~&Warning, Not LALR(1)!!: ~a, ~a --> ~{~a ~}~%"
			    (state-name state) 
			    (rule-mother (item-rule item))
			    (rule-daughters (item-rule item)))
		    (setf (item-la item) (nset-difference (item-la item) symbols-sofar)))
		  (dolist (la (item-la item))
		    (push la symbols-sofar))
		  
		  `(,(item-la item)	;case key
