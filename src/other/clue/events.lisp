...
;;; 2001-05-21  GB      - ADD-TIMER: Times with sub-second intervals specified by
;;;                       short floats were broken due to floating point arithmetric.
;;;
...
(defmacro defevent (class event-spec &rest actions)
  ...
  (let ((event-parse          (parse-event-translation event-spec actions))
    ...
    `(progn
       ...
       ;; added +symbolics by dieterm [HH 3/06/90]
       #+(or ti symbolics)
...
(defmethod translate-event ((contact basic-contact) (event event))
...
(defmethod translate-key ((contact contact) (event event))
...
(defun add-timer (contact name interval &optional data)
 ...
 The timer will be named NAME.  The event is passed DATA NAME CONTACT and DISPLAY"
  ...
  (insert-timer (make-timer
		  :name name
		  :interval (round (* interval internal-time-units-per-second))
		  :contact contact
		  :data data))
  name)
