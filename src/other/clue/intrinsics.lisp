...
;;;================
;;;
;;; [Hubertus 6/11/90]    - Genera adjustment for find-contact.
;;; [Hubertus 11/14/90]   - moved slot INITIALIZATION from CONTACT to BASIC-CONTACT.
;;; [Hubertus  1/24/91]   - reimplemented method default-options as a function.
;;;                         (see patch in defcontact.lisp)
;;;
...
(flet
  ...
  ;; display is no class but a structure in Genera version of CLX R4.3
  ;; (PCL doesn't accept structure types as specifiers)
  ;;
  (defmethod find-contact ((parent #-Genera display #+Genera t) &key name class)
...
;;;**************************************************************
;;; Modified [Hubertus  Wed Jan 23 14:14:42 1991] 
...
(defun default-options (class-name options)
  (let ((foo (get-default-options-foo class-name)))
    (if foo
	(funcall foo class-name options)
        (error "~s isn't the name of a contact subclass" class-name))))
...
(defmethod realize ((contact contact))
  ...
  (with-slots
	...
	:background       (unless input-only-p
                            (case background
                              ((NIL :none :copy)
                               background)
                              (otherwise
                                ;; Allow the user to use different
                                ;; representations for background
                                ;; colors --GB
                                (convert contact background 'xlib:pixel))))
	:depth            depth
	:allow-other-keys t
	initialization)
  ...
  (when (and (realized-p contact) (not (destroyed-p contact)))
	...
	(when (or success-p			       ; Approved or...
	  ...
	  (let ((after-effect (or success-p
	    ...
	    (when (and (functionp after-effect) #+(or lucid CLISP) (not (symbolp after-effect)) (not (composite-changing-layout-p parent)))
  ...
  (unless (destroyed-p contact)
	...
	(when (and
	  ...
	  (let
		...
		#+(or lucid CLISP) (not (symbolp after-effect))         ; incompatible change of functionp (rathke)
