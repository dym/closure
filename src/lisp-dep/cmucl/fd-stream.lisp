...
(defun next-available-buffer ()
  (let ((res (mp:without-scheduling (pop *available-buffers*))))
    (or res
        (allocate-system-memory bytes-per-buffer))))

(declaim (inline put-buffer-back))
(defun put-buffer-back (buffer)
  (let ((c (cons buffer nil)))
    (mp:without-scheduling
     (setf (cdr c) *available-buffers*
           *available-buffers* c))))
...
(define-condition io-error (stream-error)
  ((direction :reader io-error-direction :initarg :direction)
   (message :reader io-error-message :initarg :message))
  (:report
   (lambda (condition stream)
     (declare (stream stream))
     (format stream "I/O Error while ~(~A~)ing ~S: ~A."
	     (io-error-direction condition)
	     (stream-error-stream condition)
...
;;;
;;; This used to queue stuff via serve-event, I changed it to always
;;; block until everything written (or I/O timeout). --GB
;;;
(defun do-output (stream base start end reuse-sap)
  ...
  (let ((length (- end start))
    ...
    (multiple-value-bind
          (count errno)
        (unix:unix-write fd base start length)
      (cond ((not count)
             (cond ((eql errno unix:ewouldblock)
                    ;; OS said WOULDBLOCK, so arrange to wait
                    ;; until we can do the output, and try again.
                    (unless #-mp (system:wait-until-fd-usable
                                  fd :output (fd-stream-timeout stream))
                            #+mp (mp:process-wait-until-fd-usable
                                  fd :output (fd-stream-timeout stream))
                            (error 'io-timeout :stream stream :direction :write))
                    (do-output stream base start end reuse-sap))
                   (t
                    (error 'io-error
                           :stream stream
                           :direction :write
                           :message (unix:get-unix-error-msg errno)))))
            ;;
            ((not (eql count length))
             ;; not fully written, try to write the rest
             (do-output stream base (the index (+ start count)) end reuse-sap))))))
	...
	(cond
	 ((> now-needed len)
          (let ((fd (fd-stream-fd stream)))
            ...
            ;; buffer.
            ;;
            ;; Code duplication alert: yanked from do-input. --GB
            (multiple-value-bind (count errno)
              ...
              (when (or (eql count 0)
                        (and (not count) (eql errno unix:eintr)))
                (unless #-mp (system:wait-until-fd-usable fd :input (fd-stream-timeout stream))
                        #+mp (mp:process-wait-until-fd-usable fd :input (fd-stream-timeout stream))
                        (error 'io-timeout :stream stream :direction :read))))
            ;;
            (progn;;system:without-gcing -- unsure about this --GB
              (loop
                  (multiple-value-bind (count errno)
                      (unix:unix-read fd
                    ...
                    (cond ((not count)
                           (cond ((eql errno unix:ewouldblock)
                                  ;; arrange to wait until data is there
                                  (unless #-mp (system:wait-until-fd-usable fd :input (fd-stream-timeout stream))
                                          #+mp (mp:process-wait-until-fd-usable fd :input (fd-stream-timeout stream))
                                          (error 'io-timeout :stream stream :direction :read))
                                  ;; try again
                                 ...
                                 (t
                                  ...
                                  (error 'io-error :direction :read :stream stream :message (unix:get-unix-error-msg errno)))))
                          (t
                ...
                (multiple-value-bind (count errno)
                  ...
                  (cond ((not count)
                         (cond ((eql errno unix:ewouldblock)
                               ...
                               (t
                                (error 'io-error :stream stream :direction :read :message (unix:get-unix-error-msg errno)))))
                        (t
...
(defun set-routines (stream type input-p output-p buffer-p)
  (let ((target-type (case type
    ...
    (when (fd-stream-obuf-sap stream)
      (put-buffer-back (fd-stream-obuf-sap stream))
      (setf (fd-stream-obuf-sap stream) nil))
    (when (fd-stream-ibuf-sap stream)
      (put-buffer-back (fd-stream-ibuf-sap stream))
